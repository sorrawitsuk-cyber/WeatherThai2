...
const TrendIndicator = ({ current, prev, mode, size = '0.7em' }) => {
    ...
    if (diff === 0) return <span title="สถานการณ์คงที่ เทียบกับเมื่อวาน" style={{fontSize: size, opacity:0.8, color:'#94a3b8', marginLeft:'6px', whiteSpace:'nowrap'}}>➖</span>;
    ...
    const arrow = diff > 0 ? '🔺' : '🔻';
    return <span title="แนวโน้มเปรียบเทียบกับสถิติเมื่อวาน" style={{fontSize: size, color: color, opacity: 0.9, marginLeft: '6px', whiteSpace:'nowrap', fontWeight:'bold', cursor:'help'}}>{arrow}{Math.abs(diff)}</span>;
};
  ...
  const fetchWeatherByCoords = async (lat, lon) => {
    try {
      ...
      if (wRes.ok && aRes.ok) {
        setWeatherData({
          current: {
            temp: wData.current.temperature_2m, feelsLike: wData.current.apparent_temperature, humidity: wData.current.relative_humidity_2m,
            windSpeed: wData.current.wind_speed_10m, pressure: wData.current.surface_pressure, visibility: wData.current.visibility,
            uv: wData.daily.uv_index_max[0], pm25: aData.current.pm2_5, sunrise: wData.daily.sunrise[0], sunset: wData.daily.sunset[0],
            rainProb: wData.hourly.precipitation_probability[new Date().getHours()],
          },
          hourly: { time: wData.hourly.time, temperature_2m: wData.hourly.temperature_2m, precipitation_probability: wData.hourly.precipitation_probability, pm25: aData.hourly.pm2_5 },
          daily: {
            time: wData.daily.time, weathercode: wData.daily.weather_code, temperature_2m_max: wData.daily.temperature_2m_max, temperature_2m_min: wData.daily.temperature_2m_min,
            apparent_temperature_max: wData.daily.apparent_temperature_max, pm25_max: new Array(7).fill(aData.current.pm2_5), precipitation_probability_max: wData.daily.precipitation_probability_max
          },
          coords: { lat, lon }
        });
      }
  ...
  return (
    <div style={{ height: '100%', width: '100%', background: appBg, display: 'flex', justifyContent: 'center', overflowY: 'auto', fontFamily: 'Kanit, sans-serif' }} className="hide-scrollbar">
      <div style={{ width: '100%', maxWidth: isMobile ? '600px' : '1200px', display: 'flex', flexDirection: 'column', gap: '15px', padding: isMobile ? '15px' : '30px' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ background: bgGradient, borderRadius: '30px', padding: '30px 20px', color: '#fff', boxShadow: '0 20px 40px rgba(0,0,0,0.2)', position: 'relative' }}>
               <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <button onClick={() => setShowFilter(!showFilter)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', borderRadius: '50%', width: '35px', height: '35px', color: '#fff', cursor: 'pointer' }}>🔍</button>
               </div>
               ...
               <div style={{ textAlign: 'center' }}>
                  ...
                  {/* 🌟 คำอธิบายลูกศร */}
                  <div style={{ fontSize: '0.7rem', opacity: 0.8, marginTop: '12px', fontWeight: 'normal' }}>
        ...
        {/* 🌟 เพิ่มคำอธิบายหัวตาราง Top 5 */}
        <h3 style={{ margin: '10px 0 0 0', fontSize: '1.1rem', color: textColor }}>
            🇹🇭 อันดับประเทศวันนี้ <span style={{fontSize:'0.75rem', color: subTextColor, fontWeight:'normal'}}>(💡 ลูกศร 🔺🔻 เทียบกับสถิติเมื่อวาน)</span>
        </h3>
