...
export const WeatherProvider = ({ children }) => {
  ...
  // 🔑 ใส่ API KEY ของ WeatherAPI ที่นี่
  ...
  const adjustRainProb = (chance, precip_mm) => {
    ...
    if (precip_mm < 0.2) return Math.min(chance, 20); // แค่ละอองฝน กดไว้ไม่เกิน 20%
    if (precip_mm < 1.0) return Math.min(chance, 40); // ฝนตกเบาๆ กดไว้ไม่เกิน 40%
    return chance; // ฝนตกหนัก ปล่อยค่าดั้งเดิมไปเลย
  };

  // 🛠️ เครื่องมือคำนวณ AQI (US EPA) จากค่าฝุ่น PM2.5 (ป้องกันจอขาว/ค่าผิดเพี้ยน)
  ...
  // ดึง 77 จังหวัด (ใช้การดึงทีละ 10 ก้อน ป้องกัน Browser ค้าง)
  const fetchReal77Provinces = async () => {
    try {
      ...
      for (let i = 0; i < provinces77.length; i += chunkSize) {
        ...
        await new Promise(r => setTimeout(r, 600)); // พักเครื่อง 0.6 วิ
      }
  ...
  // ดึงข้อมูลละเอียดตำแหน่งปัจจุบัน
  const fetchWeatherByCoords = async (inputLat, inputLon) => {
    ...
    try {
      ...
      // WeatherAPI (3 Days Forecast + AQI)
      ...
      // --- แปลงร่างข้อมูลให้เข้ากับ UI เดิม (ป้องกันจอขาว 100%) ---
      ...
      data.forecast?.forecastday?.forEach(day => {
        day.hour?.forEach(h => {
          hTime.push(h.time);
          hTemp.push(h.temp_c);
          hPm25.push(pm25Current); // WeatherAPI ไม่มีรายชั่วโมงให้ ใช้ค่าปัจุบันแทน
        });
      });
  ...
  useEffect(() => { 
    ...
    // แอบโหลด 77 จังหวัดตามหลังเนียนๆ
